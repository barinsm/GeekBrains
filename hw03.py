...
print('Исходный массив: ', array)
minimum1 = min(array)
array.remove(minimum1)
minimum2 = min(array)
print('Первый наименьший элемент: ', minimum1)
print('Второй наименьший элемент: ', minimum2)
...
for i in range(y):
    row = []
    sum_row = 0
    print('Введите {} элемента {}-й строки: '.format(x-1, i))
    for j in range(x - 1):
        number = int(input('{}-й элемент: '.format(j)))
        sum_row += number
        row.append(number)
    row.append(sum_row)
    matrix.append(row)
print('Матрица с посчитанными суммами строк: ')
for i in matrix:
    print(i)
