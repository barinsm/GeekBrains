...
for i in range(n):
    number /= -2
    total += i+1
print('Для числа n: {} \n Сумма элементов "1+2+...+n": {} \n Равна значению "n(n+1)/2": {}'.format(n, total, n * (n + 1) / 2))
...
numbers = input('Введите через запятую произвольное количество чисел: ')
n = input('Введите цифру для поиска и подсчета: ')
total = 0
for i in numbers:
    if i == n: total += 1
